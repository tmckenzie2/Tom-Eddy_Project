...
def compute_train_test(table,part):
    '''
    This function takes in a table and a part(a fraction that determines the split of the table with) and splits the table into
    a test set and a training set for the user to manipulate later.
    '''
...
def createProbabilityList(var,totDict,priceDict,condDict,movDict,caseDict,bandDict,modDict,listDict):
        '''
...
def naive_bayes(testInstance,table):
    ...
    return max(findGreatestProbDict)

def calculatePredictiveAccuracy(testInstances,table):
    correct_predictions = 0
    for testInstance in testInstances:
        classification = naive_bayes(testInstance,table)
        if classification == testInstance[-1]:
            correct_predictions = correct_predictions + 1

    accuracy = (correct_predictions/(len(testInstances)-1)) * 100
    return accuracy
...
def main():
    ...
    for i in range(10):
        train_set,test_set = compute_train_test(table,.2)
        accuracy = calculatePredictiveAccuracy(test_set, table)
    ...
    print("NaiveBayes: accuracy =",total_accuracy, ", error rate =", 100 - total_accuracy)
